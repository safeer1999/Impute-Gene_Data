...
class NMC():
    def __init__(self, x_dim, y_dim, cfg, phase='train'):
        ...
        # loss functions
        self.recon_loss_rmse = self.build_recon_loss(self.R, self.recons, self.mask)
        #self.recon_loss_correl = self.build_recon_correl(self.R,self.recons,self.mask)
        tf.contrib.losses.add_loss(self.recon_loss_rmse)
        #tf.contrib.losses.add_loss(self.recon_loss_correl)
    ...
    #my code
    #cost function defined by 1/correlation(R,recon)

    def build_recon_correl(self,R,recon,mask) :

        correlation = tf.abs(tfp.stats.correlation(R,recon, sample_axis = 0, event_axis = None))

        #replacing nan values with 1
        nan_indices = tf.is_nan(correlation)

        correlation = tf.where(nan_indices, tf.constant(1, dtype = "float32", shape = [170]), correlation)

        correlation = tf.reduce_mean(correlation)



        self.print_correl = tf.print(correlation,output_stream = sys.stdout)

        cost_function = 1.0/correlation
    ...
    def     partial_fit(self, x, y, R, mask, lr): #fitting the model
